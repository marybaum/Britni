 ...
 * @return bool
 */
function cravings_pro_using_widgeted_template( $templates = array() ) {
...
add_action( 'admin_init', 'cravings_pro_remove_widgeted_editor' );
/**
 ...
 * @return void
 */
function cravings_pro_remove_widgeted_editor() {
	if ( cravings_pro_using_widgeted_template() ) {
		remove_post_type_support( 'page', 'editor' );
		add_action( 'admin_notices', 'cravings_pro_widgeted_admin_notice' );
	}
}
 ...
 * @return void
 */
function cravings_pro_widgeted_admin_notice() {
	printf( '<div class="updated"><p>%s</p></div>',
		__( 'The normal editor is disabled because you\'re using a widgeted page template. You need to <a href="widgets.php">use widgets</a> to edit this page.', 'cravingspro' )
	);
}
 ...
 * @return bool
 */
function cravings_pro_using_non_editor_template( $templates = array() ) {
	if ( ! isset( $_REQUEST['post'] ) ) { // Input var okay.
		return false;
	}

	if ( empty( $templates ) ) {
		$templates[] = 'filter-recipes.php';
...
add_action( 'admin_head-post.php', 'cravings_pro_maybe_remove_editor' );
/**
 * Check to make sure a widgeted page template is is selected and then disable
 * the default WordPress editor.
 *
 ...
 * @return void
 */
function cravings_pro_maybe_remove_editor() {
	if ( cravings_pro_using_non_editor_template() ) {
		remove_post_type_support( 'page', 'editor' );
	}
}

add_action( 'post_submitbox_misc_actions', 'cravings_pro_admin_meta_grid_box_view' );
/**
 ...
 * @return void
 */
function cravings_pro_admin_meta_grid_box_view( WP_Post $post ) {
	...
	if ( current_user_can( $type->cap->edit_post, $post->ID ) ) {
		$enable = cravings_pro_is_grid_enabled( $post->ID );
		require_once CRAVINGS_PRO_DIR . 'lib/admin/views/publish-meta-box.php';
	}
}

add_action( 'add_meta_boxes', 'cravings_pro_admin_add_recipe_box', 0, 2 );
/**
 ...
 * @return void
 */
function cravings_pro_admin_add_recipe_box( $post_type, $post ) {
	if ( get_page_template_slug( $post->ID ) !== 'filter-recipes.php' ) {
		return;
	}
	...
	if ( current_user_can( $type->cap->edit_post, $post->ID ) ) {
		add_meta_box(
			...
			'cravings_pro_admin_meta_recipe_box',
			'page',
			'normal',
			'high'
		);
 ...
 */
function cravings_pro_admin_meta_recipe_box( $post ) {
	require_once CRAVINGS_PRO_DIR . 'lib/admin/views/recipe-index-meta-box.php';
}
 ...
 */
function _cravings_pro_admin_meta_validate_request( $post_id, $nonce, $action ) {
...
add_action( 'save_post', 'cravings_pro_admin_meta_blog_grid_save' );
/**
 ...
 */
function cravings_pro_admin_meta_blog_grid_save( $post_id ) {
	if ( ! $valid_request = _cravings_pro_admin_meta_validate_request( $post_id, 'cravings_pro_metabox_nonce', 'save_cravings_pro_metabox' ) ) {
		return false;
	}

	$value = empty( $valid_request['_cravings_pro_enable_grid'] ) ? 'no' : 'yes';

	return (bool) update_post_meta( $post_id, '_cravings_pro_enable_grid', $value );
}


add_action( 'save_post', 'cravings_pro_admin_meta_recipe_index_save' );
/**
 * Callback function for saving our meta box data.
 *
 * @since  1.0.0
 ...
 * @return bool Whether or not data has been saved.
 */
function cravings_pro_admin_meta_recipe_index_save( $post_id ) {
	if ( ! $valid_request = _cravings_pro_admin_meta_validate_request( $post_id, 'cravings_pro_metabox_nonce', 'save_cravings_pro_metabox' ) ) {
		return false;
	}

	$slug  = '_cravings_pro_recipe_options';
