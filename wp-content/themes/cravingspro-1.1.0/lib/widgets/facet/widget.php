<?php
/**
 * CravingsPro Pro Facet WP widget.
 *
...
/**
 * CravingsPro Pro Facet widget class.
 *
 * @since   1.0.0
 * @package CravingsPro\Widgets
 */
class Cravings_Pro_Facet_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		$this->defaults = array(
			...
			'facet'       => '',
			'reset'       => '',
			'reset_text' => __( 'Reset', 'cravingspro' ),
		);

		$widget_ops = array(
			'classname'   => 'facetwp',
			'description' => __( 'Displays a FacetWP facet.', 'cravingspro' ),
		);

		$control_ops = array(
			'id_base' => 'cravings-pro-facet',
		);

		parent::__construct(
			'cravings-pro-facet',
			__( 'Cravings Pro - Facet', 'cravingspro' ),
			$widget_ops,
			$control_ops
		);
	}
	...
	protected function get_widget_template( $slug, $data = array(), $extract = false ) {
		...
		require CRAVINGS_PRO_DIR . "lib/widgets/facet/views/{$slug}.php";
	}
