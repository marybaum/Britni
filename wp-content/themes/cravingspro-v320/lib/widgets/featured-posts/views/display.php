<?php
/**
 * Cravings Pro Featured Posts Widget display markup.
 *
...
if ( $archive_grid = cravings_pro_archive_grid() ) {
	remove_filter( 'post_class', "cravings_pro_grid_{$archive_grid}" );
}
...
if ( have_posts() ) :
	...
	if ( $grid = cravings_pro_grid_exists( $data['simple_grid'] ) ) {
	...
	while ( have_posts() ) : the_post();
		...
		if ( ! empty( $data['show_title'] ) ) {
			echo '<div class="snippet"><header class="entry-header">';
		}
...
if ( $archive_grid ) {
	add_filter( 'post_class', "cravings_pro_grid_{$archive_grid}" );
}
