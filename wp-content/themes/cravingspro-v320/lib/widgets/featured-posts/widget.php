<?php
/**
 * Cravings Pro Featured Posts
 *
 * @package   CravingsPro\Widgets
 ...
 */
class Cravings_Pro_Featured_Posts extends WP_Widget {
	...
	public function __construct() {
		$this->defaults = array(
			...
			'show_title'              => 0,
			'show_byline'             => 0,
			'post_info'               => '[heart_this] <span class="dot">&middot;</span> [post_date format="M j, Y"] <span class="dot">&middot;</span> [post_comments] [post_edit]',
			'show_content'            => 'excerpt',
			'content_limit'           => '',
			'more_text'               => __( 'Read More', 'cravingspro' ),
			'more_from_category'      => '',
			'more_from_category_text' => __( 'More', 'cravingspro' ),
		);

		parent::__construct(
			'featured-post',
			__( 'Cravings Pro - Featured Posts', 'cravingspro' ),
			array(
				'classname'   => 'featured-content featuredpost cravings-featured',
				'description' => __( 'Displays featured posts with thumbnails', 'cravingspro' ),
			),
			array(
	...
	protected function get_widget_template( $slug, $data = array(), $extract = false ) {
		...
		require CRAVINGS_PRO_DIR . "lib/widgets/featured-posts/views/{$slug}.php";
	}
